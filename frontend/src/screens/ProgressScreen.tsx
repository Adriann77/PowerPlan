...
export function ProgressScreen() {
  ...
  const trainingDays = useMemo(() => {
    ...
    for (const session of sessions) {
      if (!daysMap.has(session.trainingDayId)) {
        daysMap.set(session.trainingDayId, session.trainingDayName || `Dzień ${session.trainingDayId}`);
      }
    }
      ...
      if (session.trainingDayId === selectedTrainingDayId) {
        for (const log of session.exerciseLogs) {
          if (!exercisesMap.has(log.exerciseId)) {
            exercisesMap.set(log.exerciseId, log.exerciseName || log.exerciseId);
          }
        }
      }
    }
  ...
  const exerciseProgressData = useMemo(() => {
    ...
    const dataPoints: { week: number; weight: number | null; date: string | null }[] = [];
    ...
    for (const session of sessions) {
      if (selectedTrainingDayId && session.trainingDayId !== selectedTrainingDayId) continue;
      
      const log = session.exerciseLogs.find(l => l.exerciseId === selectedExerciseId);
      if (log && log.startingWeight !== null && log.startingWeight !== undefined) {
        const point = dataPoints.find(p => p.week === session.weekNumber);
        if (point && (point.weight === null || (session.completedAt && (!point.date || new Date(session.completedAt) > new Date(point.date))))) {
    ...
    const exerciseName = availableExercises.find(e => e.id === selectedExerciseId)?.name ?? selectedExerciseId;
  ...
  }, [activePlan, sessions, selectedTrainingDayId, selectedExerciseId, availableExercises]);
    ...
    if (points.length === 0) {
      return (
        <View className="bg-slate-800 rounded-xl p-6 items-center">
          <Text className="text-gray-400">Brak danych do wyświetlenia wykresu</Text>
        </View>
      );
    }
    ...
    return (
      <View className="bg-slate-800 rounded-xl p-4 mb-4">
        ...
        <View style={{ height: chartHeight + 40, position: 'relative' }}>
          {/* Y-axis labels */}
          <View style={{ position: 'absolute', left: 0, top: 0, height: chartHeight, justifyContent: 'space-between' }}>
            <Text className="text-gray-400 text-xs">{maxWeight}kg</Text>
            <Text className="text-gray-400 text-xs">{Math.round((maxWeight + minWeight) / 2)}kg</Text>
            <Text className="text-gray-400 text-xs">{minWeight}kg</Text>
          </View>
          
          {/* Chart area */}
          <View style={{ marginLeft: CHART_PADDING, marginRight: 8, height: chartHeight, position: 'relative' }}>
            {/* Grid lines */}
            ...
            <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
              {points.map((point, index) => {
                const x = (point.week - 1) / (dataPoints.length - 1) * chartWidth;
                ...
                return (
                  <View key={point.week}>
                    ...
                    {index < points.length - 1 && (() => {
                      ...
                      const nextX = (nextPoint.week - 1) / (dataPoints.length - 1) * chartWidth;
                      const nextY = chartHeight - ((nextPoint.weight! - minWeight) / range) * chartHeight;
          ...
          {/* X-axis labels */}
          <View style={{ marginLeft: CHART_PADDING, marginRight: 8, flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }}>
            {dataPoints.map((point) => (
              <Text key={point.week} className="text-gray-400 text-xs">
                T{point.week}
              </Text>
            ))}
          </View>
          ...
          <View className="pb-8">
            ...
            {exerciseProgressData && (
              <View className="flex-row gap-3 mb-4">
                ...
                <StatCard
                  label="Postęp"
                  value={exerciseProgressData.progressPercentage !== null 
            ...
            {exerciseProgressData && (
              <View className="mt-4">
                <Text className="text-white font-bold text-lg mb-4">Szczegóły tygodniowe</Text>
                  ...
                  >
                    <View className="flex-row items-center">
                      <View className={`w-10 h-10 rounded-full items-center justify-center mr-3 ${
                        point.weight !== null ? 'bg-purple-600' : 'bg-slate-700'
                      }`}>
                        <Text className="text-white font-bold">{point.week}</Text>
                      </View>
                      <Text className="text-gray-300">Tydzień {point.week}</Text>
                    </View>
                    <View className="items-end">
                      <Text className={`font-bold ${point.weight !== null ? 'text-white text-lg' : 'text-gray-500'}`}>
